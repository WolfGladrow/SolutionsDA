...
library(latex2exp)
# png('CDFsNU160912.png',width=16,height=12,units='cm',res=300)
plot(x,CDFnormal,type='l',lwd=2,col='blue',xlab='x',ylab='CDF(x)',las=1,cex=0.4)
lines(xu,CDFuniformN,col='black',lwd=2,lty=2)
lines(xp,yp,col='red',lwd=3)
xt = -1.5
text(xt,0.9,TeX('$D_{max} = 0.0572$'),col='red',cex=1.5)
text(xt,0.7,paste('at x = ',as.character(round(xDmax,4))),col='red',cex=1.5)
# dev.off()