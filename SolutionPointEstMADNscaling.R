...
sflag = 1
if (sflag == 1) {
  # png('ExerMCmadnNormalB160819.png',width=16,height=16,units='cm',res=300)    # M = 1e4
  # png('ExerMCmadn1e5NormalB170220.png',width=16,height=16,units='cm',res=300) # M = 1e5
  plot(narr,meansdest,type='p',lwd=4,col='blue',xlab='n',
       ylab='Average MAD estimate',las=1,cex=0.6,cex.lab=1.5,
       ylim=c(min(meansdest)/1.05,0.7))
  xp = c(min(narr),max(narr)); yp = c(0.6745,0.6745)
  text(40,0.6,bquote(~M == .(M)),col='blue',cex=1.5,pos=4)
